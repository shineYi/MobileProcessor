...
unsigned int mem_read(int addr){
	...
	while(1){
		...
		if(cache[way%4][idx].sca == 1){		//second chance
			cache[way%4][idx].sca = 0;
		}

		else if(cache[way%4][idx].sca == 0){		//conflict miss
			oldest[idx]++;
			cache[way%4][idx].oldest = oldest[idx];

			if(cache[way%4][idx].dirty==0){								//not dirty
				cache[way%4][idx].valid = 1;
				cache[way%4][idx].tag = tagaddr;
               			printf("index:%x, tag:%x, valid:%d, sca:%d\n data:\n", idx, tagaddr, cache[way][idx].valid, cache[way][idx].sca);

				for(i=0; i<CLINESIZE/4; i++){
					cache[way%4][idx].data[i] = Memory[(startAddr/4)+i];
					printf("  %08x", cache[way][idx].data[i]);
					if(i%3==0)
					printf("\n");
				}
				printf("return data: cache[%d][%d] = %08x\n", way%4,idx,cache[way][idx].data[offset>>2]);
				return cache[way%4][idx].data[offset>>2];
			}

			else{		//dirty    
				cache[way%4][idx].Waddr = ((cache[way%4][idx].tag)/idx)<<4;
				for(i=0; i<CLINESIZE/4; i++)
		Memory[(cache[way%4][idx].Waddr/4)+i] = cache[way%4][idx].data[i];		
...
void mem_write(int addr, int val){
	...
	for(way=0; way<MAXWAY; way++){
		...
		if(cache[way][idx].valid == 0){		//cold miss
			...
			if(cache[way][idx].dirty==1){		//dirty    mem_write하고 다시 보기
				...
				for(i=0; i<CLINESIZE/4; i++)
					Memory[((cache[way][idx].Waddr)/4)+i] = cache[way][idx].data[i];
				printf("Write back the data to the Memory");
	...
	while(1){
		...
		if(cache[way%4][idx].sca == 1)		//second chance
			...
			oldest[idx]++;
			cache[way%4][idx].oldest = oldest[idx];
			if(cache[way%4][idx].dirty==1){		//dirty    mem_write하고 다시 보기
				//Waddr = ((cache[way][idx].tag)/idx)<<4;
				for(i=0; i<CLINESIZE/4; i++)
					Memory[((cache[way%4][idx].Waddr)/4)+i] = cache[way%4][idx].data[i];
				printf("Write back the data to the Memory");
				...
				cache[way%4][idx].data[offset>>2] = val;
				printf("Changed cache data: cache[%d][%d] = %08x\n", way%4, idx, cache[way%4][idx].data[offset>>2]);

				cache[way%4][idx].tag = tagaddr;
				cache[way%4][idx].Waddr = startAddr;
				return;
			}
